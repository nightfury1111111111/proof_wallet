...
**`window.d.ts`**
```javascript
import { Window as KeplrWindow } from "@proof-wallet/types";
...
The `@proof-wallet/types` package has the type definition related to Keplr.  
If you're using TypeScript, run `npm install --save-dev @proof-wallet/types` or `yarn add -D @proof-wallet/types` to install `@proof-wallet/types`.  
Then, you can add the `@proof-wallet/types` window to a global window object and register the Keplr related types.

> Usage of any other packages besides @proof-wallet/types is not recommended.
> - Any other packages besides @proof-wallet/types are actively being developed, backward compatibility is not in the scope of support.
...
It is recommended to use `verifyADR36Amino` function in the `@proof-wallet/cosmos` package or your own implementation instead of using `verifyArbitrary` API.  
